...
CHANNEL = "foniy_hayat"
dry_run = True // haqiqatdan ham o'chirishni xohlasangiz False qilib qo'ying
batch_size = 100
...
async def main():
    ...
    await client.start()
    print("Bot ishga tushdi. Kanal:", CHANNEL)
    to_delete_ids = []
    count_found = 0
    async for msg in client.iter_messages(CHANNEL, limit=None):
        ...
        if text and ARABIC_RE.search(text):
            count_found += 1
            print(f"Topildi (id={msg.id}): {text[:80]!r}")
            to_delete_ids.append(msg.id)
            if len(to_delete_ids) >= batch_size:
                if dry_run:
                    print(f"[dry_run] {len(to_delete_ids)} ta xabar topildi (o‘chirilmadi)")
                    to_delete_ids.clear()
                else:
    ...
    if to_delete_ids:
        if dry_run:
            print(f"[dry_run] Oxirgi partiya: {len(to_delete_ids)} ta xabar")
        else:
            await delete_batch(client, CHANNEL, to_delete_ids)
    print(f"Tugatildi. Jami arab matnli xabarlar: {count_found}")
